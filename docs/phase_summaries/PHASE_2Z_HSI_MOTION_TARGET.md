# Phase2.29 — HSI root／朝向目标迁移：最终结果

**原生HSI目标查询修正后，固定28任务仍未通过HSI独立收益与场景依赖条件；本轮停止469扩展。**
HSI能够带动人—物联合运动并保持接触，所选root／朝向目标尚未带来额外场景收益。正确HSI相对纯几何的HS均值增加4.23%、OS增加9.98%，正确场景与错配场景的差异没有建立优势。Phase2.28强基线保留。

![修正后的28任务结果](figures/PHASE_2Z_HSI_NATIVE_GOAL.png)

| 修正后28开发任务，终点修复前 | HS↓ | OS↓ | FS cm↓ | 接触%↑ | 完成数 |
|---|---:|---:|---:|---:|---:|
|纯几何关系编辑|1.177500|11.607790|0.176571|66.8160|22/28|
|正确HSI＋关系保持|1.227326|12.766820|0.179713|66.8160|22/28|
|错配HSI＋关系保持|1.223961|12.738105|0.179672|66.8160|22/28|
|正确HSI＋独立变换|1.410791|8.483406|0.180527|54.4937|22/28|

四组同一终点修复均只恢复任务19，达到23/28。关系保持相对独立变换保住12.3223个百分点接触。所有终点候选、失败与原生15项加完成率均保存；本表不与论文469聚合行作子集优劣比较。

## 条件修正及可归因范围

首次运行f4e7248采用旧共享查询路径：全部124个目标场景块位于物体目标，但模型的物体条件全部关闭。场景块与人体目标平均相距1.147710m，最大5.358083m；与原生HSI归一化0.8m人体查询位置的平均差为1.232332m。该输入不一致在封存复核中被识别，完整证据保存在首次运行analysis/goal_patch_contract.json。
3ef14e8登记一次条件修正。afc7712把is_object=false同时应用于HSI几何查询和模型输入，按原生is_mix=false规则查询人体目标，实际动态物体几何继续保留。噪声199、2次配对实现、权重.25、5cm/10°尺度、20cm/20°边界、20/40步优化及所有门槛保持原样。
修正后的124个查询位置误差均为0。相同采样下，旧/新查询的动态物体观察和其他参数精确一致，变化限定为目标场景块及其位置。纯几何28任务的全部原生指标与封存基线精确一致，来源关节误差0。正确/错配场景仍共享全部非场景条件与噪声。
修正让正确HSI的HS从1.241234降至1.227326，FS从.180706降至.179713，但OS从12.737616增至12.766820；原生输入修正没有使这个固定目标通过升级要求。首次结果和图表保留，下面的旧查询章节明确限定为首次运行。

## 修正后配对证据

10000次seed42 bootstrap；所有16个结果均有任务与场景配对区间。以下为任务95%区间，完整场景区间见紧凑结果。

| 比较/指标 | 均值差 | 任务95%区间 |
|---|---:|---:|
|正确－几何/HS|+0.049826|[-0.168600, +0.381986]|
|正确－几何/OS|+1.159030|[-0.024801, +3.014289]|
|正确－几何/FS cm|+0.003142|[-0.003488, +0.008131]|
|正确－错配/HS|+0.003365|[-0.023352, +0.030081]|
|正确－错配/OS|+0.028716|[-0.397318, +0.491021]|
|正确－错配/FS cm|+0.000041|[-0.002650, +0.002719]|

正确HSI改变27/28条动作；相对几何的root XZ坐标RMS平均21.9268mm，yaw平均绝对变化2.9363°，物体平均位移32.7649mm。手—物体坐标最大误差7.10896302e-07m，接触逐任务保持一致，首尾位置锁定通过。

## 执行、验证与下一入口

首次和修正后各完成28任务、112个主臂结果及112个终点结果，各有496次HSI前向；两轮正式运行均无作业失败。19组件检查通过；修正后运行前全套1107 passed/4 existing skips，261.46s。
修正轮控制器wall528.727s，最大allocator1.256GiB。该轮与另一项8GPU HSI训练争用GPU，启动预检已捕获占用，resource_contention.json另行记录；这些耗时作为实际成本保存，不用于速度对照。其他训练作业未被修改。
修正后运行：results/experiments/p2-mixer-hsi-motion-target-r1-s42-20260908；紧凑结果：experiments/results/p2_mixer_hsi_motion_target_native_goal_s42_20260908.json。首次运行和结果路径在旧查询记录中保留。全部原始预测、场景观察、目标、控制场、动作、拒绝候选和配对统计可追溯。
预登记caef76d，首次运行f4e7248，条件修正登记3ef14e8，修正运行afc7712；完成提交由exp/p2z-hsi-motion-target-v1定位。工程交付完成，方法升级NO-GO，原始默认路径保留。
下一会话先读本总结、OVERVIEW和最新Phase2计划。下一份具体提案应区分HSI完整预测的物理价值与root/yaw读出损失，再决定扩展关系保持的身体自由度或重做共同去噪过程。当前根据信号已实质进入联合运动、但正确场景目标没有额外收益的事实结束本设置；没有继续调权重、噪声或筛选新路由。
