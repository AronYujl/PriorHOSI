...
## 追记（2026-08-20）：采样口径订正与两个补充测量

追加节，不改上文，也不重写已封存的紧凑结果——它的 sha256 已被 P12 完成行钉住，
按 `docs/EXPERIMENT_CONVENTIONS.md` §5 封存结果只追加不重写。
Registry 行 `p1-hoi-p12-sampling-caliber-amendment-s42-20260820`。

### 订正：两个评测 cell 都是无引导的

`sampler.pelvis.guidance.enabled` 为 **false**，且 `code/priors/hoi/diffusion.py:243` 与
`code/priors/hoi/inference_guidance.py:280` 都按该标志门控。两个 run id 里的 `-eval-guided-`
来自 `tools/hoi_chain.py:85` 硬编码的默认 `--eval-tag guided`，是**误称**；
预注册第 121 行写的本来就是 unguided。

连带订正：紧凑结果里的键 `aggregate_metrics.primary_guided_r1` 名称有误导性（保留原样，不重写）；
**同口径的 D2-AI 对照行是 `d2ai_unguided` 而非 `d2ai_armb`**。按同口径重算的点估计：
P12 在 14 项里 **8 项更好、6 项更差**，其中 `contact_f1` **+3.36%**、`contact_recall` +2.98%、
`contact_acc` +2.48%——而对着有引导的 ArmB 行分别是 −0.01% / −0.79% / +0.22%。
M9 混淆不变，仍然不可归因。

### 补充测量 1：CHOIS 侧四个指标，P12 全面更差

`p1-hoi-p12-chois-matched-s42-20260820`，416/438 序列（`drop_last=True` 在 batch 32 下丢 22，
与封存运行同口径），权威机 GPU 4（先例 `p0-hoi-chois-matched-s42-20260712` 同样在权威机；
该规则禁的是吞吐失效，而 FID 不含吞吐声明）。编码器复用预训练 `finest.tar`，**未重训**。

| | FID ↓ | MatchingScore ↓ | R-Prec@1/2/3 ↑ | Diversity |
|---|--:|--:|---|--:|
| 作者发布权重 | **0.933** | **3.823** | **0.173 / 0.310 / 0.435** | 9.149 |
| P12 PRIMARY | 2.307 | 4.077 | 0.154 / 0.293 / 0.416 | 8.551 |
| P12 CONTROL | 2.817 | 4.603 | 0.142 / 0.257 / 0.358 | 8.575 |

**四个指标方向一致地更差，与物体侧（`T_e`/`T_obj`/`O_obj` 全面更好）相反。**
帧规则对 FID 的贡献是 2.817 − 2.307 = **0.510**，而到发布行的总差距是 2.307 − 0.933 = **1.374**，
故约 **0.864 无法用评测器变更解释**。

这是一条**与既有"接触参与度不足"叙事不同的**新线索：Diversity 更低、R-Precision 更差说明
生成分布本身偏窄，而"手没靠近"解释不了语义对应变差。分布级真实性更差与逐序列几何更准
同时成立是自洽的。

### 补充测量 2：batch-1 延迟持平，但成本结构相反

`p1-hoi-p12-batch1-timing-s42-20260820`，**18.157 FPS**（126 帧 / 6.9396 s，端到端 21.71 s，
排除一个预热批，CUDA 同步），对发布行的 17.779 是 **+2.1%**。

| | 采样器 | 步数 | 引导 | 场景/物体体素 |
|---|---|--:|---|---|
| 作者发布权重 | `consistency`（已蒸馏） | 16 | 开（每步反传） | 都开 |
| P12 | `diffusion`（**未蒸馏**） | 500 | 关 | 都关 |

两者都是 8 层 / 16 头，故发布行每步成本约为我们的 **31 倍**，与已封存的"引导占 guided 成本 83%"一致。
**引用这条持平必须带上两半**：P12 在未蒸馏无引导下已达持平，说明有余量；
但为补接触缺口而加引导会把这个余量吃掉。

残留混杂：17.779 测于权威机（driver 570.133.20），18.157 测于 worker（driver 580.126.09，约慢 6–7%）。
论文的 29.310 仍是第三口径——按其自身 1/8/16 步三点拟合得固定开销为负，不可比。

### 对外基线表

以上全部并入 `/data/yujinlun/report/baseline.md`，含四条必读限定
（系统级而非模型级比较、FPS 持平的成因、P12 无引导、真值地板口径）。

---

## 追记（2026-08-21）：订正——接触指标只有三项是 1.0，另三项的解析上限是 0.906392694063927

追加节，不改上文，也不重写已封存的紧凑结果——其 sha256
`08bae281fa15576d7f5bdc14eb1eaa865b8498f3b5eb5b4013170c16d4c02fab` 已被 P12 完成行与本文件
第 6、240 行钉住，按 `docs/EXPERIMENT_CONVENTIONS.md` §5 封存结果只追加不重写。
**若与上文冲突，以本节为准。** 证据索引结论 20。

### 错的值与对的值

上文（本文件 `:162`）写"四个接触指标为 1.0"。**这是错的，是我写的。**

| 指标 | 上文写的（错） | 正确值 |
|---|--:|--:|
| `contact_precision` | 1.0 | **0.906392694063927** |
| `contact_recall` | 1.0 | **0.906392694063927** |
| `contact_f1` | 1.0 | **0.906392694063927** |
| `contact_acc` | 1.0 | **1.0（正确，不订正）** |

`contact_percent` 恒等于 `gt_contact_percent` = 0.6618830180474017 也**正确，不订正**。
故"解析已定的 12 项"这个计数不变，只有其中三项的值变了。

### 成因：438 里有 41 条序列没有任何真值接触帧，而评测器把它们记为 0

`code/eval_metrics.py:311-323`：`TP + FP == 0` 时 `contact_precision = 0`，
`TP + FN == 0` 时 `contact_recall = 0`，两者都为 0 时 `contact_f1 = 0`——**真值自比也一样**，
因为那时 `FP = FN = 0` 且 `TP = gt_contact_cnt = 0`。
`code/test_infbagel_hoi.py:340` 按逐序列均值聚合，于是这 41 条以精确的 0 进入
**任何模型**的这三列，可达上限即 397/438 = **0.906392694063927**。
`contact_acc` 不受影响：零接触序列给出 `TP = FP = FN = 0`、`TN = 126`，准确率确实是 1.0。

### 这 41 条不是阈值边缘个案

最近的一条最小手-物距离 **0.07432 m**，比 0.05 m 阈值还远 **2.43 cm**；最远的 **0.62411 m**；
全部 438 条里只有 **1** 条落在阈值 1 cm 以内。用评测器自己的规则（手关节 22/23、0.05 m、
126 帧跨度）从数据集加 rest 物体网格独立复算，序列计数逐位复现，平均 `gt_contact_percent`
复算为 0.6618830180474017，与封存值差 **0.0**。另有一个更紧的跨模型独立上界：把全部 47 个已封存 438 序列 run 的
"三项同时精确为 0"集合取交集（24 个互异集合）得 **42**，恰好比 41 多一条——
`sub17_woodchair_029`，它 126 帧里只有 1 帧真值接触、最小距离 0.04549 m，
即全协议唯一那条阈值边缘序列，而本分支历史上没有任何模型命中过那一帧。

**只在这 397 条上**（指标有定义的子集）做真值往返参照，得 precision **0.9968411711880822**、
recall **0.9954630506006943**、f1 **0.9960322331956971**——残差是 FK 往返噪声，不是建模差距。
需要一个真正的天花板时，引这三个数。

### 连带的覆盖率事实

四个穿透指标只在 **181/438** 条上计算（`code/test_infbagel_hoi.py:276` 硬编码排除
woodchair、whitechair、largebox、largetable、plasticbox、trashcan）。
**这 41 条零接触序列里有 32 条同时被穿透排除**，即它们既无接触读数也无穿透读数，
却仍占着 438 的分母；只有 9 条保留穿透读数。

### 后果

本仓库任何把这三列的差距读作"离 1.0 还差多少"的说法，都**高估了约 10 个百分点**。
P12 无引导行 `contact_f1` 0.6756 对上限 0.90639 而非 1.0；Arm B 行 0.6950 同理。
门控判定不变（门控用的是与 released 行的配对比较，不是与 1.0 的距离）。
