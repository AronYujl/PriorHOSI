...
#### W3 的依赖状态（2026-08-22 用户裁决）

**前置阻塞已解除，但本次的 null 不构成 W3 的正面证据。**

`cellu-null` 界定的只有一件事：在**当前模型**上，**推理期**的接触参与度掩码即使完美也
撬不动 `contact_f1`。它对「**训练侧**几何项是否有用」零信息。前置阻塞之所以存在，
是因为 W3 的读法需要一个有效的参与度上界；把那个上界补上就解除了阻塞，
**并不同时提供任何支持 W3 的论证**。
