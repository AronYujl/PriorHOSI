...
### G. 完成：数值机制成立，R3 权重仍退化

两条 r2 retry 在同一实现提交上完成，每条覆盖 352 windows / 60 episodes × 4 timesteps ×
9 cells，共 396 个 batch 前向、12,672 条 window-cell 记录；optimizer updates 为 0。
全部预测和数值 batch readout 有限。三条 bf16 主干路径的同输入 hidden 最大差为 **0.0**，
故输出头干预没有混入主干变化。c3 下独立分项重建与生产 loss 的最大误差为
R2 `2.98e-8`、R3 `4.77e-7`；R3-off 巨大绝对 loss 的 reduction rounding 为 `4.88e-4`，
仅属于旁路描述格。32 份 episode-paired bootstrap 报告全部完成，10,000 次、seed 42。

两个 checkpoint 在四个 timestep 上都满足 C 节的三个数值机制判据。下面给出 t=0 的
R3 读数，其余 timestep 的完整结果在 compact 中，量纲为归一化表示/6D 通道空间：

| arithmetic | base | rotation L1 | bias 干预输出 RMS 差 | 人体 bias 梯度 RMS（11 批均值） |
|---|---:|---:|---:|---:|
| bf16 | 2.021580 | 2.019381 | 7.200033 | 4.0020e-5 |
| 仅 rebase fp32 | 2.021308 | 2.019111 | 7.198837 | 3.3547e-5 |
| 输出头 + rebase fp32 | 2.016813 | 2.015498 | 1.3873e-6 | 6.4259e-10 |
| 全 fp32 | 2.015641 | 2.014327 | 1.3854e-6 | 6.1491e-10 |

R3 四个 timestep 的原始 rotation 输出 RMS 为 **2770.87–2770.98**，R2 为
**0.57464–0.57545**。数千量级的共同输出再相减形成小幅旋转残差，使 bf16 量化成为
承重问题；只把已经量化的输出转成 fp32 无法恢复精度。输出头与 rebase 一起用 fp32，
R3 的 bias 干预输出差减少至少 **99.99997%**，bias 梯度减少至少 **99.99736%**。
t=0 的前向差下降为 `-7.200032`，episode-paired 95% CI `[-7.504716,-6.887270]`。
R2 在临时套用 c3 时也立即出现 bf16 bias 梯度泄漏，说明该机制在参数化本身中存在，
并非只能从退化 R3 权重上观察到。

**精度定位与权重恢复分开判定。** t=0 全 fp32、同一 c3 口径，R3 base=2.015641，
R2 base=0.050229，差 `+1.965412 [1.898458,2.033512]`。其余三个 timestep 同样显著。
R3 升级输出头精度后，位置/FK/seam 单步项改善，但 rotation L1 保持约 2.01–2.05，
没有显著下降；它承担了几乎全部基础重建误差。该 checkpoint 继续作为封存负结果。
本诊断确认共同偏移不变性破坏与反传泄漏，未重演完整训练轨迹，也没有 native rollout，
因此不将所有历史退化归因于单一数值项，不作模型晋级。

六次正式尝试（四次入口失败＋两次完成）的合计 **0.087781 GPU-h**，低于 1.0 上限；
成功 workload 的诊断计算分别为 91.39 s / 100.98 s，峰值 allocated/reserved 为
4,134,657,536 / 6,339,690,496 bytes。入口失败全部保留，未复用 run id。
前向统计为 **60 个 episode 等权均值**，梯度为 **11 个 batch 的均值**；此处没有使用
D5/D6 的总体分层权重，也不把这些单步读数当作该总体的 rollout gate。

完整 compact：`experiments/results/p1_hsi_rebase_numerics_s42_20260905.json`。
交接：`docs/phase_summaries/PHASE_1C_R3_ND.md`。R3-ND 以
...
正式生成完成：20/20条、97窗口，GT/生成身份、帧数和体型全部一致，初始两帧最大关节
误差均小于0.001cm；流水线墙钟918秒，按八卡全程占用的保守资源上界2.04 GPU-h。
20序列配对bootstrap已完成，保留全部共同数值指标，仅描述所选cohort。

首次渲染保存了20对GPU重建缓存及sit_02的5个时刻对照与两张连续姿态图，但展示验收失败：
Standard色彩变换导致亮部剪裁，全房间正面视角被餐桌遮住下肢。该渲染manifest按失败封存，
源动作、缓存和12张预览全部保留；模型生成没有重跑。为这次实际源代码修正追加一个
渲染修复提交：Filmic变换、降低照明和镜面反射、共享动作范围加场景边距的相机及侧向
视角、外墙展示剖切宽度，并使用OptiX在GPU去噪。修复只改变展示，不改人体/家具坐标。
GT末窗补齐为0.07--1.37秒，视频橙色表头和关键帧标注覆盖该区间；HTML另给录制时长内
的关节差，原始完整导出指标保留。全部候选继续渲染到独立review目录，复用原缓存。
采样/训练/评估数学与输入保持原值，component检查加真实配对渲染验证；无需重跑authority
训练suite或性能benchmark。总资源上限8 GPU-h保持不变。

### 完成：20条配对视频与五例选择

最终选择 navigate_01 / sit_04 / rise_01 / lie_02 / wash_04，录制时长内逐帧世界坐标FK
平均关节差分别为14.66 / 23.74 / 13.69 / 15.20 / 15.70cm。该差值包含合法的路径与
时序变化，不作单一质量排名。sit_04相比sit_01关节差更大，但座椅及下肢更清楚。
rise_01展示起身，尚不包含完整离开路径。躺卧三条其他候选出现更明显姿态或时序差异，
lie_03/lie_04末尾仍呈直立姿态；这些结果完整保留。

修正后的20对PNG序列和连续姿态图已全部完成。当前FFmpeg构建要求NVENC API13.1与
610驱动，本机570只支持API13.0，第一次编码未写入任何packet，空MP4与日志原样保留。
随后使用2025-08-31构建的FFmpeg7.1，复用全部渲染帧完成20条H264并排视频；解码确认
共2097帧/分支，每条15fps、1280×572，时长与输入帧数一致。最终config将编码器路径
固定为本次实际成功的兼容构建。这次环境修正与完成记录一起提交，不涉及采样代码。

交付selected.html（默认推荐五例）及index.html（全部20例），均可按动作筛选、0.5倍速
播放，并附中文选择理由、逐例差距、同时间关键帧和连续姿态对照。147.4MB离线压缩包
包含20条视频及图像、逐序列元数据、五例原始生成/GT参数。全部相机与人体/场景条件
相同，GT末窗口保持区间在视频标明。原始30fps模型/GT导出和15fps重建缓存保留。

首次曝光失败与编码兼容失败已分别封存并注册；采样只执行一次，模型输出无人工修正。
生成流水线918秒，八卡全程占用上界2.04GPU-h；首轮/修正渲染及编码的实际操作均在
登记的8GPU-h总资源上限内。定向30项通过，authority合计440通过、3跳过；渲染修正
后组件2项通过，20视频解码和HTML本地链接检查通过。

输出根：/data/yujinlun/iclr2027/artifacts/hsi/paired_train_s42_20260906/review/ 。
...
### CM1.4 完成：增加学生步数未关闭表示与物理缺口

2026-09-09，执行source 9927abf。六个正式负载均exit0：U/G单卡latency各19条/69窗，
全量U/G各375条/2271窗，一次原生Table3，以及九组×375条的八卡位置/FK重建。零训练
更新；预留成本2.754167 GPU-h，低于12上限。全部manifest在干净source上完成后统一登记。

CM16→CM25：U pen_ratio 0.0328194→0.0327330，FS 0.317062→0.316627，boundary
jerk 156.309→155.155；G为0.0274793→0.0270372、0.298000→0.298468、165.533→167.906。
上述差值95%区间均跨0。G exterior contact 320.895→311.971，差值−8.92433，逐项CI
[-16.98392,-1.67103]；八主量Bonferroni同时区间[-20.51075,0.89895]。八个同时区间均跨0。
全量和原生分组均保留，U部分穿透量改善与G交互接触下降同时记录。

FID U 20.19524→23.01966，差值+2.82441 CI[0.80394,5.26486]；G 22.90380→23.65130，
差值+0.74751 CI[-1.51791,2.83127]，两组R@3变化均不确定。25步生成成本增加U57.72%、
G38.66%；FPS83.7322/13.9159，生成秒/序列1.79427/10.76676，端到端1.86635/10.84758。

部署位置/FK主量（cm）：GT0.43238；R2 U/G0.95248/1.04784；DDIM25 0.94729/1.01186；
CM16 1.72761/1.72400；CM25 1.74250/1.72559。CM16−R2为U+0.77513 CI[0.74074,0.81255]，
G+0.67616 CI[0.63492,0.71845]；教师相对GT也明确增加。CM25−CM16两格CI跨0。root平移
误差约数值零；内部同样存在分歧。3375条记录、七项标量及28关节均值、八组配对全部完成。
batch128重建forward6.05–6.25ms，峰值allocated261.1MB/reserved274.7MB；完整八卡负载29秒。

U全量及holdout无>5g/低骨盆walk；G全量4条/9帧>5g、holdout2条/6帧、零低骨盆walk，
守卫通过。所有异常ID和最大边界退化案例保留。14个注册配对报告及语义/接触补充全部完成。
同时区间最终文件primary_multiple_comparisons_v2.json使用与paired_bootstrap.py相同的
default_rng42，逐项区间已核对一致；初始RandomState统计稿单独保留。正式负载无失败。

2.81审计已封存：原始代码/JSON来自训练bundle真实样本拆分均值，旧报告1/n外推脚注有误。
R2 U/G FID38.41515/40.04968；CG差值CI跨0，主要分布差距在无CG时已存在。位置FID改善
与FK身体质量仍需分别判定。固定16/25步排查步数；CM与DDIM之间仍有权重及再加噪公式差异。

报告/compact：experiments/results/p1_hsi_cm1_steps_representation_s42_20260909.{md,json}；
交接docs/phase_summaries/PHASE_1C_CM1_REPRESENTATION.md。保留CM16和R2+CG固定对照。
...
### 2026-09-09 R4.1 用户修订：GPU4–7直接从头训练

原队列15:51:14UTC自动启动在0/1/6/7；epoch0完整恢复点写出后在15:58:01UTC暂停，
用户随后明确选择“直接从头gpu4-7训练”，替代布局实测和续训方案。旧进程已全部终止，
原manifest登记aborted；保留656更新、epoch0完整恢复状态和日志，不将此中止当作科学负结果。
旧运行实际计算及保存0.452592GPU-h，含暂停期间保留显存的资源成本0.494249GPU-h，
补充成本文件reservation_cost_addendum.json保留在旧manifest旁。

新run p1-hsi-r4-fk-train-fresh47-s42-20260909 在物理GPU4/5/6/7上从随机初始化：
load_state_dict=false、resume_from为空、start_epoch=0；146255更新/299530240窗口完整预算，
权重0.24812712291771177、4×512、seed42、LR/warmup/cosine/EMA和条件均保持注册值。
GPU4推理进程当前约1.7GiB，其余三卡将释放旧训练占用；启动前重新记录实时preflight。
...
### CM2.1完成：同状态终点定义对齐后，学生身体与边界误差仍明确增加

两source各60条/364窗全部完成，499/279/59共2184状态；7份配对及family8同时区间齐全。
120条native粗帧与封存输出逐值一致，全部状态数组有限且历史完全一致。R2原生轨迹从三
捕获时刻继续所得终点逐值相同、最大差0，验证中间x0与终点区分及剩余轨迹实现。

在59，教师单步/学生到同一教师终点的body误差(cm)：R2source0.51516/1.69487，CMsource
0.57799/1.77660；差+1.17971[1.02732,1.36022]、+1.19860[1.01662,1.43032]（family8）。
两source、279/59、body/边界速度共8项同时CI全部为正。低噪声body两source均成立，按
注册选择身体终点保真目标提案分支。学生root误差1.674/1.730cm，旋转4.294/4.669度；
不能只归因旋转头。全部首窗/生成历史、分层/加权和其他误差原值完整保留。

原生60队列CM−DDIM边界jerk+29.26078 CI[22.27865,36.94151]，有效接触−48.45269
CI[−88.68723,−13.51195]；pen/FS/interior jerk变化不确定，改善点估计全部保留。
两source均0个>5g及低骨盆walk案例。所有结论限定已暴露开发队列，未作全375晋级验收。

两GPU任务/merge全exit0、零训练更新。6份端点统计及family8完成后，native汇总对
未定义朝向指标求均值失败；保留null/有效数0后复用6份报告、补齐native第7份及汇总。
原失败日志exit1和恢复exit0均保留。初始成本附注读取到不支持的文件birth time=0，
final_completion.json修正该字段，旧附注保留。GPU任务0.855685GPU-h，含统计保守上界
共0.862817GPU-h<12；显存最小实测余量13524MiB。authority480/3、组件19，无源码
变化后额外重跑。18配置及最终registry校验保留。

报告/compact：experiments/results/p1_hsi_cm2_alignment_s42_20260911.{md,json}；交接
PHASE_1C_CM2_ALIGNMENT.md。报告中给出一个待批准候选：保留CM1配方，只增加最低三格
19/39/59的全未来body22（含root）FK教师终点保真，目标最多3步冻结R2 DDIM，单次
...
八rank零更新校准exit0，所有项的loss/梯度/cosine有限；低噪声行数35/24/31/24/32/28/27/27。
trunk10%规则给1.373943740373，rotation-head25%规则给1.251875724992，
按注册取后者。全部原值和一页中位数表封存在p1_hsi_cm2_body_calibration_s42_20260911。
...
### 2026-09-12 CM2.3资源修订已批准：总上限161GPU-h，补齐三项

用户批准把总资源上限从160调整到161GPU-h。已有实耗160.017709，剩余约0.982291；
本次只执行已注册且未创建manifest的latency-unguided、latency-guided、gate。三个实际
Hydra入口重新解析结果与已归档job配置一致，模型、seed、样本、采样与统计规则保持。
学生时延在physical GPU1/logical cuda:0串行执行，记录仍存在的外部推理竞争；教师时延
直接引用旧记录。已有6个成功任务、1个失败及37份配对/4份缓存FID差全部复用。
本提交由实际资源批准和未开始负载的source转换触发。完整验收结束再统一封存结论。

### 2026-09-12 CM2.3完整验收完成：联合门失败，保留R2+CG

用户随后明确批准168GPU-h作为最终资源上限，取代中间161上限。已注册的学生U/G时延及
组合gate全部exit0，复用全部既有质量/特征/表示结果与37份配对、4份缓存FID差。
九个固定GPU负载成功，原position-fk入口失败独立保留；完整验收没有遗漏已注册指标。

GPU1、batch1固定19条69窗（5预热、14计时），ID与教师时延对照完全相同，CUDA同步。
U/G平均generation FPS=94.518030/19.039375，生成秒/episode=1.591602/7.851533，
端到端=1.655440/7.916283。前后GPU1有4214MiB的其它推理进程，解释为当次竞争运行；
独占速度和相对历史空闲教师的受控加速比尚未建立。有引导当次速度未达到20FPS。

完整G的25个relative bounds为13 PASS/5 FAIL/7 INCONCLUSIVE；安全门失败，速度当次未达。
物理/安全失败独立于速度条件，因此CM2不晋级。FID/MM正结果及R@3不确定、body21分歧
增加、所有完整/holdout异常案例、固定中途到终点读数全部保留。直接位置/FK分歧不是同
状态教师终点误差，本次没有新增CM2.1探针；不把该负结果单独归因于某一训练机制。

实际总160.084340GPU-h<最终168上限，含训练156.488370、准备0.735828、评估2.860143；
原表示失败0.005768计入。最后三项新增0.066631GPU-h；原160停点超额和两次资源批准
保留。无runtime变化，沿用authority487/3及定向66，最终做registry/配置/覆盖核对。
质量/表示source deef2d8，剩余时延/gate source072fa5b，runtime相同，全部manifest已终结。
...
### CM2.4完成：身体终点改善，首个未来帧仍有保真缺口

两源各60条/364窗，2184状态全部完成；新旧x_t/previous_clean/history/条件及教师
单步/终点逐值一致，学生历史一致且全部数组有限。120条原生粗帧、每条68个原生字段
均逐值复现。16份探针身份保持R2 teacher与CM2 student；源cm16始终为CM1。

CM1→CM2 t59 body误差(cm)：R2source1.694874→1.540839，CM1source1.776597→1.641616；
主差−0.154035[−0.211885,−0.094992]与−0.134981[−0.201577,−0.073197]（family8）。
279两body差也明确改善。8项主比较为4改善、1退化、3不确定。退化项是CM1source t59
边界速度，0.143158→0.152896m/s，差0.009738[0.001838,0.017535]；R2source同项不确定。
t59 root/全局FK差逐项区间跨0，旋转约−0.21度，逐项改善。全部499、其他误差、分层、
人口加权、首窗/后续窗及12份配对保留。首窗速度点估计改善、后续窗退化，分解仅作描述。

共同两历史帧令e0=e1=0，因此探针边界速度差=e2/0.1s，首个边界加速度差=e2/(0.1s)²。
该速度平方目标属于首个未来帧位置的定向重加权；jerk还包含下一未来帧。按注册分支
提出保留CM2、在19/39/59增加首个未来帧24关节绝对FK教师终点软约束（含root），共享
现有教师终点；保留14帧body22目标、同预算/初始化/推理与完整验收。一次梯度校准只
形成待审阅提案；新配置、run id和训练均未创建。训练到生成历史的迁移风险保留。

两个GPU负载/合并及统计均exit0，无正式失败。26组件测试、18完全解析配置、12配对
及GPU family8核对齐全；runtime保持原值，沿用authority487/3。R2source0.439083、
CM1source0.372014、统计0.003920，总0.815017GPU-h<4；最小实测余量10830MiB，8卡
外部竞争原样保留。零训练更新，源rollout不作为CM2新原生评估，不作部署速度结论。
