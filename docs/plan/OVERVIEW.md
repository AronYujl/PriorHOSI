...
2026-09-07 当前进度：Phase2.21完成A缓存归因/B终点语义/C第一窗口控制。
HOIPrior对±10cm waypoint有稳定响应：22适用任务的B1后root有向响应4.68/4.70cm，
物体5.28/5.21cm；两侧所有任务方向正确。但联合质量门槛NO-GO，双侧全部实用
保护通过仅4/22；接触锚点和部分场景代理置信上界超限。D完整路线池、E HSI评分未启动。
旧候选第一窗口raw差异已仅.99mm，B1后1.02mm；无保护HS oracle空间也仅.659%。
新物体三维/实际末帧终点计算与112条旧原生数值完全一致。1028 tests通过/4历史skips；
首轮遗漏B1参考调用的计数失败保留，修正r2完成75窗口及全部28视频/轨迹。
详见 [Phase2.21总结](../phase_summaries/PHASE_2S_WAYPOINT_CONTROL.md)
及 [Phase2计划](PHASE_2_COMPOSITION.md)。唯一优先问题是条件可控后的联合可行性。
等待新交接定义人—物关系和场景质量的条件侧保护；当前不扩大2.22/2.23或441/469。
旧Phase2.20/Temporal负结果保持封存，尚未建立完整469超越InfBaGel的结论。
