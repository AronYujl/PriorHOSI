...
2026-09-08 当前进度：Phase2.33可行方向诊断完成。相同约束内纯几何小步23改善/0退化，HS下降2.90%；正确HSI均值近零变化且劣于错配，局部场景信号条件失败。导数分解将正确HSI的主要下降贡献定位到被固定手足约束去掉的分量。保留Phase2.28强几何默认路径；下一入口按用户可接受小幅退化的偏好，具体登记HSI接入最优链及全469对照。[本阶段总结](../phase_summaries/PHASE_2AD_HSI_FEASIBLE_DIRECTION.md)。
