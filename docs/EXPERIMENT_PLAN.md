...
该 gate 已通过：预测、exact-identity GT 和带标签的并排视频均为完整 300 帧、
30 FPS、10 秒，GT 相机 fit bounds 由预测 manifest 锁定；预测与 GT 均走向场景
右后方扶手椅并完成坐下。抽查未见明显悬空或严重穿透，但单一斜俯视角不能排除轻微
座椅碰撞，因此产物保留原始 motion 并将该限制写入可视化记录。
