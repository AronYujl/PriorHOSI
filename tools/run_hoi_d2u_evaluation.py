#!/usr/bin/env python3
"""Evaluate the D2-U final online checkpoint against the completed D2-T control."""
...
RUN_ID = "p1-hoi-d2u-native-eval-s42-20260721"
SUBPHASE = "1B-D2-U0-eval"
CONTROL_CHECKPOINT_SHA256 = "1543af304acf76f385dbd3656a1ca82ea25dcd504ee120f7f63e821d71483647"
CONTROL_AGGREGATE_SHA256 = "8862cfdc013482bc4fb3810bfbcaf3131010b3ee9bee13324dccd27df85e2702"
CONTROL_PER_SEQUENCE_SHA256 = "422dd0e8bc87cc896186ebdaea9fcd22868ae4ba40d2d61d5bb16a1000500f9e"
...
def resolve_target(args) -> str:
    ...
    if "${" in completed.stdout:
        raise ValueError("D2-U evaluation config contains unresolved interpolation")
...
def classify(
    ...
    *,
    ...
    if not contract_passed:
        classification = "balanced-objective-contract-failure-stop"
    elif not mechanism_passed:
        classification = "balanced-objective-negative-stop"
    elif not effective_passed:
        classification = "balanced-objective-positive-but-not-effective-stop"
    else:
...
def validate_training_result(args) -> Dict[str, object]:
    if args.target_checkpoint.name != (
        "p1-hoi-d2u-balanced-author-update-s42-20260721_windows006144000.pth"
    ):
        raise ValueError("D2-U evaluation requires the registered final checkpoint basename")
    ...
    checks = {
        ...
        "run_id": metrics.get("run_id") == "p1-hoi-d2u-balanced-author-update-s42-20260721",
        ...
        "processed_windows": metrics.get("processed_windows") == 6144000,
        "optimizer_updates": metrics.get("optimizer_updates") == 3000,
    ...
    matching = [
        ...
        if item.get("processed_windows") == 6144000
    ...
    if failed:
        raise ValueError(f"D2-U training artifact contract mismatch: {failed}")
...
def run_target(args) -> None:
    ...
    if paths["evaluation"].exists() or paths["log"].exists():
        raise FileExistsError("refusing to overwrite D2-U target evaluation")
    environment = dict(os.environ)
    environment["ROOT_DIR"] = str(REPO)
    ...
    if completed.returncode:
        raise RuntimeError(f"D2-U author-native evaluation exited {completed.returncode}")
...
def main() -> None:
    ...
    if args.run_id != RUN_ID:
        raise ValueError(f"D2-U evaluation run id must be {RUN_ID}")
    if not __import__("re").fullmatch(r"[0-9a-f]{64}", args.target_sha256):
        raise ValueError("D2-U target checkpoint SHA-256 must be lowercase hexadecimal")
    if not __import__("re").fullmatch(r"[0-9a-f]{64}", args.training_metrics_sha256):
        raise ValueError("D2-U training metrics SHA-256 must be lowercase hexadecimal")
    if args.python.resolve() != Path(os.environ.get("INFBAGEL_PYTHON", "")).resolve():
        raise ValueError("D2-U evaluation requires the absolute INFBAGEL_PYTHON")
    if os.environ.get("INFBAGEL_WORKER_EXPERT") != "hoi" or socket.gethostname() != "node01":
        raise RuntimeError("D2-U evaluation is restricted to the HOI worker")
    if args.resolve_only:
        prepare_resolved_config(args)
        return
    if git_output("status", "--porcelain=v1", "--untracked-files=all"):
        raise RuntimeError("D2-U evaluation refuses a dirty worker checkout")
    if load_json(args.resolved_config.resolve()) != resolved_config(args):
        raise ValueError("D2-U runtime arguments do not match archived resolved config")
    ...
    try:
        ...
        if actual_hashes != expected_hashes:
            raise ValueError(f"D2-U runtime artifact hash mismatch: {actual_hashes}")
