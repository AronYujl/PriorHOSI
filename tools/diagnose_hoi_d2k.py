#!/usr/bin/env python3
"""Run the preregistered Phase 1B D2-K0 sealed-AdamW routing audit."""
...
RUN_ID = "p1-hoi-d2k-adamw-routing-s42-20260716"
...
def prepare_blocks(dataset: PriorWindowDataset, positions: Sequence[int]):
    if len(positions) != PRIMARY_WINDOWS or len(positions) % BLOCK_SIZE:
        raise ValueError("D2-K requires exactly 128 windows in fixed 16-window blocks")
    return [
...
def _loss_tensors(losses: Mapping[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
    ...
    result["human_reconstruction"] = result["joint_position"] + result["joint_rotation"]
    result["object_reconstruction"] = result["object_translation"] + result["object_rotation"]
    result["reconstruction"] = result["human_reconstruction"] + result["object_reconstruction"] + result["contact"]
...
def gradient_block(
    ...
    *,
    ...
    generator.manual_seed(stable_seed(f"D2K:primary:{timestep}:{block_index}"))
    ...
    base_gradients = {
        name: torch.autograd.grad(components[name], parameters, retain_graph=True, allow_unused=True)
    ...
    geometry = routing_geometry(
        base_gradients, direct_total, parameters, optimizer_states, optimizer_group, groups,
    )
    ...
    result["finite"] = bool(
        result["finite"] and all(math.isfinite(value) for value in loss_values.values())
    )
    del batch, noisy, prediction, losses, components, base_gradients, direct_total
    return result


def audit_checkpoint(name, model, raw_optimizer, diffusion, dataset, prepared, device):
    model.eval()
    ...
    if any(parameter.grad is not None for parameter in parameters):
        raise RuntimeError("D2-K model unexpectedly has populated .grad buffers")
    timesteps = {}
    finite = True
    maximum_gradient_error = 0.0
    maximum_clip_error = 0.0
    maximum_adamw_error = 0.0
    for timestep in TIMESTEPS:
        ...
        maximum_gradient_error = max(maximum_gradient_error, max(
            block["total_gradient_formula_relative_l2"] for block in blocks
        ))
        maximum_clip_error = max(maximum_clip_error, max(
            block["clipping"]["formula_replay_max_abs"] for block in blocks
        ))
        maximum_adamw_error = max(maximum_adamw_error, max(
    ...
    optimizer_after = optimizer_state_sha256(raw_optimizer)
    mapped_after = mapped_state_sha256(states)
    return {
        "timesteps": timesteps,
        "finite": bool(finite),
        "optimizer_contract": contract,
        "optimizer_contract_exact": True,
        "maximum_total_gradient_formula_relative_l2": maximum_gradient_error,
        "maximum_clipping_formula_replay_abs": maximum_clip_error,
        "maximum_adamw_decomposition_relative_l2": maximum_adamw_error,
        ...
        "mapped_state_sha256_after": mapped_after,
...
def resolved_config(args: argparse.Namespace) -> Dict[str, object]:
    return {
        ...
        "phase": "p1",
        "subphase": "1B-D2-K0",
        "mode": "sealed-adamw-counterfactual-routing-audit",
        "repo_root": str(REPO),
        "seed": 42,
        "device": args.device,
        ...
        "gate": {
            ...
            "human_delta_bootstrap_lower_min": 0.05,
            "adamw_human_bootstrap_lower_min": 0.15,
            "adamw_object_bootstrap_lower_min": 0.15,
        },
...
def main() -> None:
    ...
    if args.run_id != RUN_ID:
        raise ValueError(f"D2-K0 run id must be {RUN_ID}")
    if args.block_size != BLOCK_SIZE:
        raise ValueError(f"D2-K0 block size must be {BLOCK_SIZE}")
    ...
    if json.loads(config_path.read_text(encoding="utf-8")) != config:
        raise ValueError("runtime arguments do not match archived D2-K0 resolved config")
    if os.environ.get("INFBAGEL_WORKER_EXPERT") != "hoi":
        raise RuntimeError("D2-K0 requires INFBAGEL_WORKER_EXPERT=hoi")
    if subprocess.check_output(["git", "status", "--porcelain"], cwd=REPO, text=True).strip():
        raise RuntimeError("D2-K0 refuses a dirty worker checkout")
    if sha256_file(REPO / "data/train/norm.npy") != EXPECTED_NORMALIZATION_SHA256:
        raise ValueError("D2-K0 normalization hash mismatch")
    if sha256_file(REPO / "code/bps.pt") != EXPECTED_BPS_SHA256:
        raise ValueError("D2-K0 BPS hash mismatch")
    checkpoint_paths = {
    ...
    if device.type != "cuda" or not torch.cuda.is_available():
        raise RuntimeError("D2-K0 is a four-GPU-worker CUDA diagnostic")
    seed_everything(42)
    started = time.time()
    dataset = PriorWindowDataset(
    ...
    if selection["sha256"] != EXPECTED_PRIMARY_SHA256:
        raise ValueError("D2-K0 primary selection hash mismatch")
    if selection["terminal_windows"] != 0 or selection["skipped_terminal_ranks"] != [768, 770]:
        raise ValueError("D2-K0 nonterminal scan contract mismatch")
    ...
    output: Dict[str, object] = {
        ...
        "selection": {
            ...
            "skipped_terminal_ranks": selection["skipped_terminal_ranks"],
        },
        "selection_disjoint_from_d2h0_d2i0_d2j0": True,
        "assets": {
            "data_contract_sha256": EXPECTED_DATA_CONTRACT_SHA256,
            "normalization_sha256": EXPECTED_NORMALIZATION_SHA256,
            "bps_sha256": EXPECTED_BPS_SHA256,
    ...
    for name in CHECKPOINTS:
        ...
        checkpoint_parameter_names = tuple(name_ for name_ in raw["model"] if name_ in dict(model.named_parameters()))
    ...
    output["paired_noise_identity"] = bool(all(
        output["candidates"]["R-1024"]["timesteps"][str(timestep)]["blocks"][block]["q_noise_sha256"]
        == output["candidates"]["R-3072"]["timesteps"][str(timestep)]["blocks"][block]["q_noise_sha256"]
