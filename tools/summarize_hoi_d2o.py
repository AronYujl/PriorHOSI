#!/usr/bin/env python3
"""Create the tracked compact aggregate for a completed Phase 1B D2-O0 run."""
...
def validate_identity(metrics: Mapping[str, object], manifest: Mapping[str, object]) -> None:
    if metrics.get("run_id") != RUN_ID or manifest.get("experiment_id") != RUN_ID:
        raise ValueError("D2-O summary run-id mismatch")
    if metrics.get("git_commit") != manifest.get("git", {}).get("commit"):
        raise ValueError("D2-O summary manifest/workload commit mismatch")
    if metrics.get("status") != "completed":
        raise ValueError("D2-O summary requires completed metrics")
