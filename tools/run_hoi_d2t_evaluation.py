#!/usr/bin/env python3
"""Evaluate the completed D2-T online checkpoint against the sealed R1024 control."""
...
RUN_ID = "p1-hoi-d2t-native-eval-s42-20260721"
SUBPHASE = "1B-D2-T0-eval"
TARGET_SHA256 = "1543af304acf76f385dbd3656a1ca82ea25dcd504ee120f7f63e821d71483647"
CONTROL_CHECKPOINT_SHA256 = "ded9a12d4e85179c37e2457475649ccc614ef364b97eaebade0629b2c11d4ed8"
CONTROL_AGGREGATE_SHA256 = "d95d3090455e763159a4cac793301f9f4744837bf60b4ed21eaef4a141c9ad2b"
CONTROL_PER_SEQUENCE_SHA256 = "11c11fcd90c0ce2e67d705bb64c3a78bbe2b0e9f84aff7fcb57cab25087e2a1f"
...
def resolve_target(args) -> str:
    ...
    if "${" in completed.stdout:
        raise ValueError("D2-T evaluation config contains unresolved interpolation")
...
def classify(
    ...
    *,
    ...
    if not contract_passed:
        classification = "author-update-rule-contract-failure-stop"
    elif not mechanism_passed:
        classification = "author-update-rule-negative-stop"
    elif not effective_passed:
        classification = "positive-but-not-effective-diffusion-stop"
    else:
...
def run_target(args) -> None:
    ...
    if paths["evaluation"].exists() or paths["log"].exists():
        raise FileExistsError("refusing to overwrite D2-T target evaluation")
    environment = dict(os.environ)
    environment["ROOT_DIR"] = str(REPO)
    ...
    if completed.returncode:
        raise RuntimeError(f"D2-T author-native evaluation exited {completed.returncode}")
...
def main() -> None:
    ...
    if args.run_id != RUN_ID:
        raise ValueError(f"D2-T evaluation run id must be {RUN_ID}")
    if args.target_sha256 != TARGET_SHA256:
        raise ValueError("D2-T target checkpoint requested hash mismatch")
    if args.python.resolve() != Path(os.environ.get("INFBAGEL_PYTHON", "")).resolve():
        raise ValueError("D2-T evaluation requires the absolute INFBAGEL_PYTHON")
    if os.environ.get("INFBAGEL_WORKER_EXPERT") != "hoi" or socket.gethostname() != "node01":
        raise RuntimeError("D2-T evaluation is restricted to the HOI worker")
    if args.resolve_only:
        prepare_resolved_config(args)
        return
    if git_output("status", "--porcelain=v1", "--untracked-files=all"):
        raise RuntimeError("D2-T evaluation refuses a dirty worker checkout")
    if load_json(args.resolved_config.resolve()) != resolved_config(args):
        raise ValueError("D2-T runtime arguments do not match archived resolved config")
    ...
    try:
        ...
        if actual_hashes != expected_hashes:
            raise ValueError(f"D2-T runtime artifact hash mismatch: {actual_hashes}")
        ...
        validate_candidate_result("target", target_aggregate, target_per_sequence, TARGET_SHA256)
