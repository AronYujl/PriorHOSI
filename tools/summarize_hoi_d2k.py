#!/usr/bin/env python3
"""Create the tracked compact aggregate from a sealed D2-K0 artifact tree."""
...
def validate_run_identity(metrics, manifest, resolved) -> None:
    ...
    if identifiers != (RUN_ID, RUN_ID, RUN_ID):
        raise ValueError(f"D2-K0 run-id mismatch across sealed artifacts: {identifiers}")


def _mean(values: Sequence[float]) -> float:
    array = np.asarray(values, dtype=np.float64)
    if not len(array) or not np.isfinite(array).all():
        raise ValueError("D2-K0 compact summary received nonfinite values")
...
def _geomean_nonnegative(values: Sequence[float]) -> float:
    ...
    if not len(array) or not np.isfinite(array).all() or bool((array < 0).any()):
        raise ValueError("D2-K0 compact geomean received invalid values")
    if bool((array == 0).any()):
        return 0.0
...
def _mean_cosine(records, matrix, first, second):
    ...
    return {
        "value": _mean(defined) if defined else 0.0,
        "defined_blocks": len(defined),
    }
    ...
    for group in PARAMETER_GROUPS:
        ...
        groups[group] = {
            ...
            "direction_loss_cosine_mean": {
                direction: {
                    loss: _mean_cosine(records, "direction_loss_cosine", direction, loss)
                    for loss in LOSS_COMPONENTS
                } for direction in DIRECTIONS
            },
            ...
            "adamw_minus_clipped_efficiency_mean": {
                loss: _mean([
                    record["adamw_minus_clipped_efficiency"][loss] for record in records
                ]) for loss in LOSS_COMPONENTS
            },
        }
    return {
        ...
        "maximum_adamw_decomposition_relative_l2": max(
            float(block["adamw_decomposition_relative_l2"]) for block in blocks
        ),
        "clipping": {
            key: _mean([block["clipping"][key] for block in blocks])
            for key in ("preclip_norm", "clip_coefficient", "postclip_norm")
        },
        "stored_lr": _mean([block["stored_lr"] for block in blocks]),
        "loss_values_mean": {
...
def compact_candidate(candidate: Mapping[str, object]) -> Dict[str, object]:
    return {
        ...
        "optimizer_contract": candidate["optimizer_contract"],
        "optimizer_contract_exact": candidate["optimizer_contract_exact"],
        ...
        "maximum_adamw_decomposition_relative_l2": candidate[
            "maximum_adamw_decomposition_relative_l2"
        ],
        "model_state_sha256_before": candidate["model_state_sha256_before"],
        "model_state_sha256_after": candidate["model_state_sha256_after"],
        "optimizer_state_sha256_before": candidate["optimizer_state_sha256_before"],
        "optimizer_state_sha256_after": candidate["optimizer_state_sha256_after"],
        "mapped_state_sha256_before": candidate["mapped_state_sha256_before"],
        "mapped_state_sha256_after": candidate["mapped_state_sha256_after"],
...
def main() -> None:
    ...
    if any(not path.is_file() for path in paths.values()):
        ...
        raise FileNotFoundError(f"D2-K0 artifact is missing: {missing}")
    ...
    if metrics["git_commit"] != manifest["git"]["commit"]:
        raise ValueError("D2-K0 metrics/manifest Git commit mismatch")
    ...
    output = {
        ...
        "selection_disjoint_from_d2h0_d2i0_d2j0": metrics[
            "selection_disjoint_from_d2h0_d2i0_d2j0"
        ],
