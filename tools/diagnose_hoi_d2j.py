#!/usr/bin/env python3
"""Run the preregistered Phase 1B D2-J0 global-clipping routing audit."""
...
def prepare_blocks(dataset: PriorWindowDataset, positions: Sequence[int]):
    if len(positions) != PRIMARY_WINDOWS or len(positions) % BLOCK_SIZE:
        raise ValueError("D2-J requires exactly 128 windows in fixed 16-window blocks")
    return [
...
def gradient_block(
    ...
    *,
    ...
    generator.manual_seed(stable_seed(f"D2J:primary:{timestep}:{block_index}"))
    ...
    geometry = clip_gradient_geometry(base_gradients, direct_total, groups)
...
def audit_checkpoint(model, diffusion, dataset, prepared, device) -> Dict[str, object]:
    model.eval()
    parameters, groups = parameter_group_indices(model)
    before = state_dict_sha256(model)
    if any(parameter.grad is not None for parameter in parameters):
        raise RuntimeError("D2-J model unexpectedly has populated .grad buffers")
    timesteps = {}
    finite = True
    ...
    for timestep in TIMESTEPS:
        blocks = [
            gradient_block(
                ...
                timestep=timestep, block_index=index, device=device,
            )
        ...
        maximum_clipping_error = max(
            maximum_clipping_error,
            max(block["clipping"]["formula_replay_max_abs"] for block in blocks),
        )
    ...
    return {
        ...
        "maximum_clipping_formula_replay_abs": maximum_clipping_error,
...
def resolved_config(args: argparse.Namespace) -> Dict[str, object]:
    return {
        ...
        "phase": "p1",
        "subphase": "1B-D2-J0",
        "mode": "frozen-production-gradient-clip-routing-audit",
        "repo_root": str(REPO),
        "seed": 42,
        "device": args.device,
        ...
        "gate": {
            "gradient_formula_relative_l2_max": 1e-5,
            "clipping_formula_replay_max_abs": 1e-6,
            "preclip_norm_bootstrap_lower_min": 50.0,
            "clip_coefficient_bootstrap_upper_max": 0.02,
            "human_efficiency_bootstrap_upper_max": 0.15,
            "object_efficiency_bootstrap_lower_min": 0.15,
        },
...
def main() -> None:
    ...
    if args.run_id != RUN_ID:
        raise ValueError(f"D2-J0 run id must be {RUN_ID}")
    if args.block_size != BLOCK_SIZE:
        raise ValueError(f"D2-J0 block size must be {BLOCK_SIZE}")
    ...
    if json.loads(config_path.read_text(encoding="utf-8")) != config:
        raise ValueError("runtime arguments do not match archived D2-J0 resolved config")
    if os.environ.get("INFBAGEL_WORKER_EXPERT") != "hoi":
        raise RuntimeError("D2-J0 requires INFBAGEL_WORKER_EXPERT=hoi")
    if subprocess.check_output(["git", "status", "--porcelain"], cwd=REPO, text=True).strip():
        raise RuntimeError("D2-J0 refuses a dirty worker checkout")
    if sha256_file(REPO / "data/train/norm.npy") != EXPECTED_NORMALIZATION_SHA256:
        raise ValueError("D2-J0 normalization hash mismatch")
    if sha256_file(REPO / "code/bps.pt") != EXPECTED_BPS_SHA256:
        raise ValueError("D2-J0 BPS hash mismatch")
    checkpoint_paths = {
    ...
    if device.type != "cuda" or not torch.cuda.is_available():
        raise RuntimeError("D2-J0 is a four-GPU-worker CUDA diagnostic")
    seed_everything(42)
    started = time.time()
    dataset = PriorWindowDataset(
    ...
    if selection["sha256"] != EXPECTED_PRIMARY_SHA256:
        raise ValueError("D2-J0 primary selection hash mismatch")
    if selection["terminal_windows"] != 0:
        raise ValueError("D2-J0 primary cohort must be nonterminal")
    ...
    output: Dict[str, object] = {
        ...
        "selection_disjoint_from_d2i0": True,
        "assets": {
            "data_contract_sha256": EXPECTED_DATA_CONTRACT_SHA256,
            "normalization_sha256": EXPECTED_NORMALIZATION_SHA256,
            "bps_sha256": EXPECTED_BPS_SHA256,
