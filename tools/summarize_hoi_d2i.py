#!/usr/bin/env python3
"""Create the tracked compact aggregate from a sealed D2-I0 artifact tree."""
...
from priors.gradient_routing import CHECKPOINTS, LOSS_COMPONENTS, PARAMETER_GROUPS, TIMESTEPS  # noqa: E402
...
def validate_run_identity(
    metrics: Mapping[str, object], manifest: Mapping[str, object], resolved: Mapping[str, object],
) -> None:
    identifiers = (
        metrics.get("run_id"), manifest.get("experiment_id"), resolved.get("run_id"),
    )
    if identifiers != (RUN_ID, RUN_ID, RUN_ID):
        raise ValueError(f"D2-I0 run-id mismatch across sealed artifacts: {identifiers}")


def _mean(values: Sequence[float]) -> float:
    array = np.asarray(values, dtype=np.float64)
    if not len(array) or not np.isfinite(array).all():
        raise ValueError("D2-I0 compact summary received nonfinite values")
...
def _geomean_nonnegative(values: Sequence[float]) -> float:
    ...
    if not len(array) or not np.isfinite(array).all() or bool((array < 0).any()):
        raise ValueError("D2-I0 compact geomean received invalid values")
    if bool((array == 0).any()):
        return 0.0
...
def compact_blocks(blocks: Sequence[Mapping[str, object]]) -> Dict[str, object]:
    ...
    for group in PARAMETER_GROUPS:
        ...
        groups[group] = {
            ...
            "total_over_reconstruction_norm_ratio_geometric_mean": _geomean_nonnegative([
                record["total_over_reconstruction_norm_ratio"] for record in records
            ]),
            "total_reconstruction_cosine_mean": _mean([
                record["total_reconstruction_cosine"]["value"] for record in records
            ]),
            "gradient_cancellation_index_mean": _mean([
                record["gradient_cancellation_index"] for record in records
            ]),
        }
    return {
        "blocks": len(blocks),
...
def compact_candidate(candidate: Mapping[str, object]) -> Dict[str, object]:
    return {
        ...
        "cohorts": {
            cohort: {
                "timesteps": {
                    str(timestep): compact_blocks(
                        candidate["cohorts"][cohort]["timesteps"][str(timestep)]["blocks"]
                    )
...
def main() -> None:
    ...
    if any(not path.is_file() for path in paths.values()):
        ...
        raise FileNotFoundError(f"D2-I0 artifact is missing: {missing}")
    ...
    if metrics["git_commit"] != manifest["git"]["commit"]:
        raise ValueError("D2-I0 metrics/manifest Git commit mismatch")
