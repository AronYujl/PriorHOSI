#!/usr/bin/env python3
"""Create the tracked compact aggregate for a completed Phase 1B D2-R0 run."""
...
def validate_identity(
    ...
    if (
        ...
        or manifest.get("experiment_id") != RUN_ID
    ):
        raise ValueError("D2-R summary run-id mismatch")
    if metrics.get("git_commit") != manifest.get("git", {}).get("commit"):
        raise ValueError(
            "D2-R summary manifest/workload commit mismatch"
        )
    if metrics.get("status") != "completed":
        raise ValueError("D2-R summary requires completed metrics")
