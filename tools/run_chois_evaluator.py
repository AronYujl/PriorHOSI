...
def _embeddings(loader: DataLoader, wrapper: Any, metrics: Mapping[str, Any], matching: bool) -> Dict[str, Any]:
    ...
    with torch.no_grad():
        for batch in loader:
            word_embeddings, pos_one_hots, _, sent_lens, motions, motion_lengths, _ = batch
            if matching:
                ...
                matching_score_sum += float(distance.trace())
                top_k_count += metrics["top_k"](np.argsort(distance, axis=1), top_k=3).sum(axis=0)
    ...
    result: Dict[str, Any] = {"motion_embeddings": np.concatenate(all_motion_embeddings, axis=0)}
    if matching:
...
def evaluate(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    truth_loader = _loader(
        PathConfiguredCHOISEvaluationDataset(args.ground_truth.resolve(), args.data_root.resolve(), vectorizer),
        args.batch_size, args.workers,
    )
    prediction_loader = _loader(
        PathConfiguredCHOISEvaluationDataset(args.predictions.resolve(), args.data_root.resolve(), vectorizer),
        args.batch_size, args.workers,
    )
